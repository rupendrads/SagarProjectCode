...
import requests
import json
from async_market import MDSocket_io
import logging
import urllib3
...
async def bulk_insert_to_db(buffer, table_name, combined_token_names, pool):
    #print(f'i  nside bulk_insert_to_db ')
...
=======
                        datetime.fromtimestamp(item.get("LastUpdateTime", None) + 315532800) - timedelta(minutes=time_difference) if item.get("LastUpdateTime", None) is not None else None,
>>>>>>> 813dca723f1f8d6e40dbf0cda0543c3b74810a65
                        item.get("Close", None),
...
async def get_data_queue(data_queue, table_name, end_time, combined_token_names, pool, soc, subscribed_symbol, market_token):
    #logger.log(f'Called get_data_queue {data_queue.qsize()}')
    buffer = []
    ...
    while True:
        ...
        if datetime.now().hour >= end_time.time().hour:
            print('Time is over, exiting')
            try:
                if soc.sid.connected:
                    soc.disconnect()
                if (data_queue.qsize()>0):
                    print(f'BULK_INSERT_SIZE changed to 1: {data_queue.qsize()}')
                    logger.log(f'BULK_INSERT_SIZE changed to 1: {data_queue.qsize()}')
                    BULK_INSERT_SIZE = 1
                else:
                    return
            except Exception as e:
                logger.log(f"Error while disconnecting: {e}")
            break
