...
async def evaluate_strategy(queue: Queue, symbol : str) -> None:
    ...
    if tradebooks:
        ...
        collective_tradebook.to_csv('tradebook.csv',index=False)
...
async def main() -> None:
    ...
    tradebook_path = 'tradebook.csv'       #strategyname_tradebook_datetimestamp(ddmmyyhourmin)
    analyzer = Analyzer(tradebook_path)
    df = pd.read_csv(tradebook_path)
    # df.to_csv(f'{moving_crossover_strategy.__name__}tradebook{str(datetime.now().date)}.csv',index=False)
    analyzer.plot_data()
